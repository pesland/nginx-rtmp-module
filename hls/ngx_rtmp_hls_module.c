...
ngx_rtmp_hls_write_playlist(ngx_rtmp_session_t *s)
{
    ...
    for (i = 0; i < ctx->nfrags; i++) {
        ...
        p = ngx_slprintf(p, end,
                         "#EXTINF:%.3f,\n"
                         "https://baykemal-opensocial.googleusercontent.com/gadgets/proxy?container=focus&refresh=31536000&url=https://cdn.baykemalkazanacak.workers.dev/hls/%V%V%s%uL.js\n",
